...
return new class extends Migration
{
    public function up(): void
    {
        DB::connection('pgsql')->update("
            UPDATE bioestadistica.record_values
            SET value_json = jsonb_set(value_json, '{rows}', '{}'::jsonb)
            WHERE jsonb_typeof(value_json->'rows') = 'array'
        ");
        ...
        DB::connection('pgsql')->statement(<<<'SQL'
            ...
            CROSS JOIN LATERAL jsonb_each(
                CASE
                    WHEN jsonb_typeof(rv.value_json->'rows') = 'object' THEN rv.value_json->'rows'
                    ELSE '{}'::jsonb
                END
            ) row_value
            CROSS JOIN LATERAL jsonb_each(
                CASE
                    WHEN jsonb_typeof(row_value.value) = 'object' THEN row_value.value
                    ELSE '{}'::jsonb
                END
            ) metric
            WHERE f.type = 'tabla'
