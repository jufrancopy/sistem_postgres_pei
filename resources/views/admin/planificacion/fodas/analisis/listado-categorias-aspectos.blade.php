                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="card text-center">
                                <a href="{{ route('foda-analisis-ambiente-interno', $perfil->id) }}"
                                    class="btn btn-success">Analizar otra Categoria</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @foreach ($analisis as $valor)
        {
            name: '{{ $valor->aspecto->name }}' + 
                '@switch($valor->tipo)@case('Fortaleza') <p class="badge badge-success"> Fortaleza </p>' + 
                    ...
                    '@case('Debilidad') <p class="badge badge-danger"> Debilidad </p> @break' + 
                    '@case('Amenaza') <p class="badge badge-warning"> Amenaza </p> @break' + 
                    '@default <p> (Pendiente de Análisis) </p> @endswitch',
            children: 
            [
                {
                    name: '<table class="table table-bordered">' +
                        ...
                        @switch($valor->ocurrencia)
                            @case('0.1')
                            '<td>Baja ({{ $valor->ocurrencia }})</td>' +
                            @break

                            @case('0.3')
                            '<td>Media ({{ $valor->ocurrencia }})</td>' +
                            @break

                            @case('0.5')
                            '<td>Alta ({{ $valor->ocurrencia }})</td>' +
                            @break

                            @case('0.7')
                            '<td>Muy Alta ({{ $valor->ocurrencia }})</td>' +
                            @break

                            @case('0.9')
                            '<td>Cierta ({{ $valor->ocurrencia }})</td>' +
                            @break

                            @default
                            '<td>(Pendiente de Análisis)</td>' +
                        @endswitch

                        @switch($valor->impacto)
                            @case('0.05')
                            '<td>Muy Bajo ({{ $valor->impacto }})</td>' +
                            @break

                            @case('0.1')
                            '<td>Bajo ({{ $valor->impacto }})</td>' +
                            @break

                            @case('0.2')
                            '<td>Moderado ({{ $valor->impacto }})</td>' +
                            @break

                            @case('0.4')
                            '<td>Alto ({{ $valor->impacto }})</td>' +
                            @break

                            @case('0.8')
                            '<td>Muy Alto ({{ $valor->impacto }})</td>' +
                            @break
                        ...
                        @switch ($total)
                            ...
                            @case($total >= 0.18)
                            '<td><strong class="badge badge-success"> ({{ $total }})</strong></td>' +
                            @break

                            @case($total <= 0.17)
                            '<td><strong class="badge badge-danger">Insuficiente ({{ $total }})</strong></td>' +
                            @break
                        ...
                        '<td><a href="{{ route('foda-analisis.edit', $valor->id) }}">Analizar</a></td>' +
                        '<td>' +
                        '{!! Form::open(['route' => ['foda-analisis.destroy', $valor->id],'method' => 'DELETE','style' => 'display:inline',]) !!}' +
                        '<button class="btn btn-danger btn-circle" onclick="return confirm(\'Estas seguro de eliminar el análisis?\')">' +
