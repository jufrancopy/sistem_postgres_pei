                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-info">
                            <h4 class="card-title ">Aspectos de la Categoría {{ $categoria->name }}</b>

                                <div class="pull-right">
                                    <a class="btn btn-warning"
                                        href="{{ route('foda-analisis-ambiente-interno', $idPerfil) }}"> Atras</a>
                                </div>
                        </div>
                        @if (count($errors) > 0)
                                    ...
                                    <div class="form-group">
                                        ...
                                        @if (count($aspectos) > 1)
                                            ...
                                            @foreach ($aspectos as $value)
                                                <label>{{ Form::checkbox('aspecto_id[]', $value->id, in_array($value->id, $aspectosChecked) ? true : false, ['class' => 'name']) }}
                                                    {{ $value->name }}</label>
                                                <br />
                                            @endforeach
                                    </div>
                        ...
                        @else
                            <div class="col-xs-12 col-sm-12 col-md-12 text-center">
                                <button type="submit" class="btn btn-success"><a
                                        href="{{ route('foda-analisis-ambiente-interno', $idPerfil) }}">Sin
                                        Aspectos</a></button>
                            </div>
                            @endif
                        </div>
