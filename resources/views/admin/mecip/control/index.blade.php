        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 14px;">
            ...
            <div class="modal-body p-4 text-dark">
                ...
                <div class="p-4 bg-light rounded border text-center my-3" style="border-radius: 12px !important;">
                    ...
                    @php
                        $apiUrl = url('/api/mecip/importar-json');
                        $bookmarkletJs = "javascript:(function(){var url=window.location.href;var pageText=document.body.innerText||'';var casoMatch=url.match(/(?:m_process_id|searchclient_process_id)=(\d+)/i)||pageText.match(/(?:caso|expediente|nº|no\.)\s*[:#]?\s*(\d{5,10})/i)||[0,'3782431'];var numeroCaso=casoMatch[1]||'3782431';var macroproceso='Gestión Estratégica';var proceso='Monitoreo y Seguimiento de la Planificación Estratégica';var subproceso='Monitoreo del Plan Estratégico Institucional';var codigoSubproceso='GES_002_01';var version='2';var macroMatch=pageText.match(/Macroporoceso:\s*([^\n\r\t<]+)/i);if(macroMatch)macroproceso=macroMatch[1].trim();var procMatch=pageText.match(/Proceso:\s*([^\n\r\t<]+)/i);if(procMatch)proceso=procMatch[1].trim();var subMatch=pageText.match(/Subproceso\/Procedimiento:\s*([^\n\r\t<]+)/i);if(subMatch)subproceso=subMatch[1].trim();var codMatch=pageText.match(/Código:\s*\(b\)\s*([A-Z0-9_\-]+)/i);if(codMatch)codigoSubproceso=codMatch[1].trim();var verMatch=pageText.match(/Versión:\s*(\d+)/i);if(verMatch)version=verMatch[1].trim();var productos=[];var insumos=[];var tables=document.querySelectorAll('table');tables.forEach(function(tbl){var txt=tbl.innerText||'';if(txt.includes('Clientes')||txt.includes('Producto')){var rows=tbl.querySelectorAll('tr');rows.forEach(function(r,idx){if(idx>0){var tds=r.querySelectorAll('td');if(tds.length>=3){productos.push({codigo:tds[0]?.innerText.trim(),nombre:tds[1]?.innerText.trim(),descripcion:tds[2]?.innerText.trim(),cliente:tds[3]?.innerText.trim()||'Consejo de Administración'});}}});}if(txt.includes('Proveedores')||txt.includes('Insumo')){var rows=tbl.querySelectorAll('tr');rows.forEach(function(r,idx){if(idx>0){var tds=r.querySelectorAll('td');if(tds.length>=3){insumos.push({codigo:tds[0]?.innerText.trim(),nombre:tds[1]?.innerText.trim(),descripcion:tds[2]?.innerText.trim(),proveedor:tds[3]?.innerText.trim()||'Todas las direcciones'});}}});}});var actividades=[];if(productos.length>0){productos.forEach(function(p,i){actividades.push({codigo:p.codigo||('ACT_0'+(i+1)),nombre:p.nombre||('Actividad de '+subproceso),responsable:'Analista IPS',tareas:[{descripcion:p.descripcion||'Ejecución del procedimiento',tiempo_minutos:30}]});});}else{actividades.push({codigo:'ACT_01',nombre:subproceso,responsable:'Analista IPS',tareas:[{descripcion:'Modelado y revisión del procedimiento MECIP IPS',tiempo_minutos:45}]});}var payload={numero_caso:numeroCaso,codigo_subproceso:codigoSubproceso,macroproceso:macroproceso,proceso:proceso,subproceso:subproceso,version:version,productos:productos,insumos:insumos,actividades:actividades};fetch('".$apiUrl."',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({json_data:JSON.stringify(payload)})}).then(r=>r.json()).then(data=>{if(data.success){alert('✅ ¡Caso #'+payload.numero_caso+' (Subproceso '+payload.codigo_subproceso+') Replicado con Éxito en SIPLAN!\nAbriendo expediente...');window.open(data.redirect,'_blank');}else{alert('⚠️ Error al replicar: '+(data.message||'Error'));}}).catch(err=>{var form=document.createElement('form');form.method='POST';form.action='".$apiUrl."';form.target='_blank';var i2=document.createElement('input');i2.type='hidden';i2.name='json_data';i2.value=JSON.stringify(payload);form.appendChild(i2);document.body.appendChild(form);form.submit();alert('⚡ Replicando expediente Caso #'+payload.numero_caso+' a SIPLAN...');});})();";
                    @endphp
