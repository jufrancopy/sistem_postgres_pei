            ...
            <div class="modal-body p-4 text-dark">
                ...
                <div class="p-4 bg-light rounded border text-center my-3" style="border-radius: 12px !important;">
                    ...
                    @php
                        ...
                        $bookmarkletJs = "javascript:(function(){function cleanTxt(s){if(!s)return'';s=s.trim();if(s.includes('Replicar')||s.includes('SIPLAN')||s.includes('Marcador')||s.includes('Mostrando')||s.includes('Acciones')||s.includes('Volver al graficador')||s.includes('Despliegue'))return'';return s;}function getCellTxt(td){if(!td)return'';var divs=td.querySelectorAll('div');if(divs&&divs.length>1){var arr=[];divs.forEach(function(d){var t=cleanTxt(d.innerText);if(t)arr.push(t);});return arr.join('\\n');}return cleanTxt(td.innerText);}var url=window.location.href;var pageText=document.body.innerText||'';var casoMatch=url.match(/(?:m_process_id|searchclient_process_id)=(\d+)/i)||pageText.match(/(?:caso|expediente|nº|no\.)\s*[:#]?\s*(\d{5,10})/i)||[0,'3782431'];var numeroCaso=casoMatch[1]||'3782431';var macroproceso='PROCESO DE GOBERNANZA IPS';var proceso='GESTIÓN ESTRATÉGICA E INSTITUCIONAL';var subproceso='Modelado y Gestión de Procesos MECIP IPS';var codigoSubproceso='GES-BPM-'+numeroCaso;var version='1.0';var macroMatch=pageText.match(/Macrop?oroceso:\s*([^\n\r\t<]+)/i);if(macroMatch&&cleanTxt(macroMatch[1]))macroproceso=cleanTxt(macroMatch[1]);var procMatch=pageText.match(/Proceso:\s*([^\n\r\t<]+)/i);if(procMatch&&cleanTxt(procMatch[1]))proceso=cleanTxt(procMatch[1]);var subMatch=pageText.match(/Subproceso(?:\/Procedimiento)?:\s*([^\n\r\t<]+)/i)||pageText.match(/(?:Subproceso|Asunto|Procedimiento):\s*([^\n\r\t<]+)/i);if(subMatch&&cleanTxt(subMatch[1]))subproceso=cleanTxt(subMatch[1]);var verMatch=pageText.match(/Versi[oó]n:\s*([^\n\r\t<]+)/i);if(verMatch&&cleanTxt(verMatch[1]))version=cleanTxt(verMatch[1]);var productos=[];var insumos=[];var actividades=[];var tareasMap={};if(window.angular){try{var el=document.querySelector('cy-vista-formatos')||document.querySelector('[data-ng-controller]')||document.querySelector('.ng-scope');if(el){var ctrl=(window.angular.element(el).controller&&window.angular.element(el).controller())||(window.angular.element(el).scope&&window.angular.element(el).scope().\$ctrl)||(window.angular.element(el).scope&&window.angular.element(el).scope());if(ctrl&&ctrl.formatos){ctrl.formatos.forEach(function(fr){if(fr.numero==46&&fr.datos_formato){fr.datos_formato.forEach(function(fp){var car=Array.isArray(fp.caracteristicas)?fp.caracteristicas.map(function(c){return typeof c==='object'?(c.descripcion||c.nombre||''):c;}).join('\n'):(fp.caracteristicas||'');productos.push({codigo:fp.codigo||'',nombre:fp.producto||fp.nombre||fp.codigo,descripcion:car,cliente:fp.cliente||fp.clientes||'Consejo de Administración'});});}if(fr.numero==47&&fr.datos_formato){fr.datos_formato.forEach(function(fp){var car=Array.isArray(fp.caracteristicas)?fp.caracteristicas.map(function(c){return typeof c==='object'?(c.descripcion||c.nombre||''):c;}).join('\n'):(fp.caracteristicas||'');insumos.push({codigo:fp.codigo||'',nombre:fp.insumo||fp.nombre||fp.codigo,descripcion:car,proveedor:fp.proveedor||fp.proveedores||'Todas las direcciones'});});}if(fr.numero==48&&fr.datos_formato){fr.datos_formato.forEach(function(fp){actividades.push({codigo:fp.codigo||'',nombre:fp.actividad||fp.nombre||fp.codigo,objetivo:fp.objetivo||'',responsable:fp.cargo||fp.responsable||'Analista IPS',tareas:[]});});}});}}}}catch(e){}}if(productos.length===0||insumos.length===0||actividades.length===0){var tables=document.querySelectorAll('table');tables.forEach(function(tbl){var txt=(tbl.innerText||'').toLowerCase();var rows=tbl.querySelectorAll('tbody tr');if(!rows.length)rows=tbl.querySelectorAll('tr');if(productos.length===0&&(txt.includes('producto')||txt.includes('cliente')||txt.includes('46'))){rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){var cod=cleanTxt(tds[0]?.innerText);var nom=cleanTxt(tds[1]?.innerText);if(nom&&nom!=='Volver al graficador'&&!nom.includes('graficador')){productos.push({codigo:cod,nombre:nom,descripcion:getCellTxt(tds[2]),cliente:cleanTxt(tds[3]?.innerText)||'Consejo de Administración'});}}});}if(insumos.length===0&&(txt.includes('insumo')||txt.includes('proveedor')||txt.includes('47'))){rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){var cod=cleanTxt(tds[0]?.innerText);var nom=cleanTxt(tds[1]?.innerText);if(nom&&nom!=='Volver al graficador'&&!nom.includes('graficador')){insumos.push({codigo:cod,nombre:nom,descripcion:getCellTxt(tds[2]),proveedor:cleanTxt(tds[3]?.innerText)||'Todas las direcciones'});}}});}if(actividades.length===0&&txt.includes('actividad')&&(txt.includes('objetivo')||txt.includes('responsable')||txt.includes('48'))){rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){var cod=cleanTxt(tds[0]?.innerText);var nom=cleanTxt(tds[1]?.innerText);if(nom&&nom!=='Volver al graficador'&&!nom.includes('graficador')){actividades.push({codigo:cod,nombre:nom,objetivo:cleanTxt(tds[2]?.innerText),responsable:cleanTxt(tds[3]?.innerText)||'Analista IPS',tareas:[]});}}});}if(txt.includes('tarea')&&txt.includes('tiempo')){var currAct='';rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=4){currAct=cleanTxt(tds[1]?.innerText);var tDesc=cleanTxt(tds[2]?.innerText);var tTime=cleanTxt(tds[3]?.innerText);if(currAct&&tDesc){if(!tareasMap[currAct])tareasMap[currAct]=[];tareasMap[currAct].push({descripcion:tDesc,tiempo:tTime});}}else if(tds.length===2&&currAct){var tDesc2=cleanTxt(tds[0]?.innerText);var tTime2=cleanTxt(tds[1]?.innerText);if(tDesc2){if(!tareasMap[currAct])tareasMap[currAct]=[];tareasMap[currAct].push({descripcion:tDesc2,tiempo:tTime2});}}});}});}if(actividades.length>0){actividades.forEach(function(act){var nom=act.nombre.toLowerCase();var matched=[];for(var key in tareasMap){if(key.toLowerCase().includes(nom.substring(0,25))||nom.includes(key.toLowerCase().substring(0,25))){matched=tareasMap[key];break;}}if(matched.length>0){act.tareas=matched.map(function(t){var timeMin=30;if(/(\d+)\s*d[ií]as?/i.test(t.tiempo)){timeMin=parseInt(t.tiempo.match(/(\d+)/)[1])*480;}else if(/(\d+)\s*horas?/i.test(t.tiempo)){timeMin=parseInt(t.tiempo.match(/(\d+)/)[1])*60;}else if(/(\d+)\s*min/i.test(t.tiempo)){timeMin=parseInt(t.tiempo.match(/(\d+)/)[1]);}return{descripcion:t.descripcion,tiempo_minutos:timeMin};});}else{act.tareas=[{descripcion:act.objetivo||'Ejecución del procedimiento',tiempo_minutos:30}];}});}if(actividades.length===0){if(productos.length>0){productos.forEach(function(p,i){actividades.push({codigo:p.codigo||('ACT_0'+(i+1)),nombre:p.nombre||('Actividad de '+subproceso),responsable:'Analista IPS',tareas:[{descripcion:p.descripcion||'Ejecución del procedimiento',tiempo_minutos:30}]});});}else{actividades.push({codigo:'ACT_01',nombre:subproceso,responsable:'Analista IPS',tareas:[{descripcion:'Modelado y revisión del procedimiento MECIP IPS',tiempo_minutos:45}]});}}var payload={numero_caso:numeroCaso,codigo_subproceso:codigoSubproceso,macroproceso:macroproceso,proceso:proceso,subproceso:subproceso,version:version,productos:productos,insumos:insumos,actividades:actividades};var jsonStr=JSON.stringify(payload);if(navigator.clipboard){navigator.clipboard.writeText(jsonStr);}fetch('".$importJsonUrl."',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:jsonStr}).then(function(r){return r.json();}).then(function(res){if(res.success&&res.redirect){window.location.href=res.redirect;}else{window.open('".$targetRedirectUrl."?import_json='+encodeURIComponent(jsonStr),'_blank');}}).catch(function(){window.open('".$targetRedirectUrl."?import_json='+encodeURIComponent(jsonStr),'_blank');});})();";
                    @endphp
