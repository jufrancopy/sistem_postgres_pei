            ...
            <div class="modal-body p-4 text-dark">
                ...
                <div class="p-4 bg-light rounded border text-center my-3" style="border-radius: 12px !important;">
                    ...
                    @php
                        ...
                        $bookmarkletJs = "javascript:(function(){function cleanTxt(s){if(!s)return'';s=s.trim();if(s.includes('Replicar')||s.includes('SIPLAN')||s.includes('Marcador')||s.includes('Mostrando')||s.includes('Acciones'))return'';return s;}function getCellTxt(td){if(!td)return'';var divs=td.querySelectorAll('div');if(divs&&divs.length>1){var arr=[];divs.forEach(function(d){var t=cleanTxt(d.innerText);if(t)arr.push(t);});return arr.join('\\n');}return cleanTxt(td.innerText);}var url=window.location.href;var pageText=document.body.innerText||'';var casoMatch=url.match(/(?:m_process_id|searchclient_process_id)=(\d+)/i)||pageText.match(/(?:caso|expediente|nº|no\.)\s*[:#]?\s*(\d{5,10})/i)||[0,'3782431'];var numeroCaso=casoMatch[1]||'3782431';var macroproceso='PROCESO DE GOBERNANZA IPS';var proceso='GESTIÓN ESTRATÉGICA E INSTITUCIONAL';var subproceso='Modelado y Gestión de Procesos MECIP IPS';var codigoSubproceso='GES-BPM-'+numeroCaso;var version='1.0';var macroMatch=pageText.match(/Macrop?oroceso:\s*([^\n\r\t<]+)/i);if(macroMatch&&cleanTxt(macroMatch[1]))macroproceso=cleanTxt(macroMatch[1]);var procMatch=pageText.match(/Proceso:\s*([^\n\r\t<]+)/i);if(procMatch&&cleanTxt(procMatch[1]))proceso=cleanTxt(procMatch[1]);var subMatch=pageText.match(/Subproceso(?:\/Procedimiento)?:\s*([^\n\r\t<]+)/i)||pageText.match(/(?:Subproceso|Asunto|Procedimiento):\s*([^\n\r\t<]+)/i);if(subMatch&&cleanTxt(subMatch[1]))subproceso=cleanTxt(subMatch[1]);var verMatch=pageText.match(/Versi[oó]n:\s*([^\n\r\t<]+)/i);if(verMatch&&cleanTxt(verMatch[1]))version=cleanTxt(verMatch[1]);var productos=[];var insumos=[];var actividades=[];if(window.angular){try{var ngEl=document.querySelector('[ng-app]')||document.querySelector('.ng-scope')||document.body;var sc=angular.element(ngEl).scope();if(sc){var scData=sc.vm||sc;if(scData.productos&&scData.productos.length)productos=scData.productos;if(scData.insumos&&scData.insumos.length)insumos=scData.insumos;if(scData.actividades&&scData.actividades.length)actividades=scData.actividades;}}catch(e){}}var tables=document.querySelectorAll('table');tables.forEach(function(tbl){var txt=tbl.innerText||'';if(txt.includes('Clientes')||txt.includes('Producto')){var rows=tbl.querySelectorAll('tbody tr');if(!rows.length)rows=tbl.querySelectorAll('tr');rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){productos.push({codigo:cleanTxt(tds[0]?.innerText),nombre:cleanTxt(tds[1]?.innerText)||cleanTxt(tds[0]?.innerText),descripcion:getCellTxt(tds[2]),cliente:cleanTxt(tds[3]?.innerText)||'Consejo de Administración'});}});}if(txt.includes('Proveedores')||txt.includes('Insumo')){var rows=tbl.querySelectorAll('tbody tr');if(!rows.length)rows=tbl.querySelectorAll('tr');rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){insumos.push({codigo:cleanTxt(tds[0]?.innerText),nombre:cleanTxt(tds[1]?.innerText)||cleanTxt(tds[0]?.innerText),descripcion:getCellTxt(tds[2]),proveedor:cleanTxt(tds[3]?.innerText)||'Todas las direcciones'});}});}if(txt.includes('Actividad')&&(txt.includes('Objetivo')||txt.includes('Responsable'))){var rows=tbl.querySelectorAll('tbody tr');if(!rows.length)rows=tbl.querySelectorAll('tr');rows.forEach(function(r){var tds=r.querySelectorAll('td');if(tds.length>=2){actividades.push({codigo:cleanTxt(tds[0]?.innerText),nombre:cleanTxt(tds[1]?.innerText)||cleanTxt(tds[0]?.innerText),objetivo:cleanTxt(tds[2]?.innerText),responsable:cleanTxt(tds[3]?.innerText)||'Analista IPS',tareas:[{descripcion:cleanTxt(tds[2]?.innerText)||\'Ejecución del procedimiento\',tiempo_minutos:30}]});}});}});if(actividades.length===0){if(productos.length>0){productos.forEach(function(p,i){actividades.push({codigo:p.codigo||('ACT_0'+(i+1)),nombre:p.nombre||('Actividad de '+subproceso),responsable:'Analista IPS',tareas:[{descripcion:p.descripcion||'Ejecución del procedimiento',tiempo_minutos:30}]});});}else{actividades.push({codigo:'ACT_01',nombre:subproceso,responsable:'Analista IPS',tareas:[{descripcion:'Modelado y revisión del procedimiento MECIP IPS',tiempo_minutos:45}]});}}var payload={numero_caso:numeroCaso,codigo_subproceso:codigoSubproceso,macroproceso:macroproceso,proceso:proceso,subproceso:subproceso,version:version,productos:productos,insumos:insumos,actividades:actividades};var jsonStr=JSON.stringify(payload);if(navigator.clipboard){navigator.clipboard.writeText(jsonStr);}fetch('".$importJsonUrl."',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:jsonStr}).then(function(r){return r.json();}).then(function(res){if(res.success&&res.redirect){window.location.href=res.redirect;}else{window.open('".$targetRedirectUrl."?import_json='+encodeURIComponent(jsonStr),'_blank');}}).catch(function(){window.open('".$targetRedirectUrl."?import_json='+encodeURIComponent(jsonStr),'_blank');});})();";
                    @endphp
