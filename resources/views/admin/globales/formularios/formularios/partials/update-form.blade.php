            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td colspan="{{ $value->colspan }}">
                        ...
                        <label>{{ Form::checkbox('value[]', $value->id, in_array($value->id, $variablesChecked) ? true :
                            false, array('class' => 'name')) }} --}}
                            @foreach ($variablesChecked as $v )
                                {{$v}}
                            @endforeach

                            {{ Form::checkbox('value[]', $value->id, in_array($value->id, $variablesChecked) ? true :
                            false, array('class' => 'name')) }}
                            {{-- {{ $value->name }} --}}
