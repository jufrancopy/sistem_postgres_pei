    ...
    public function handle()
    {
        ...
        try {
            foreach ($bioEstablecimientos as $bio) {
                ...
                $datos = [
                    'nombre_oficial'          => $bio->nombre,
                    'departamento'            => $departamento,
                    'nro_departamento'        => $nroDepto ?: 18,
                    'microred'                => $bio->microred?->nombre ?? ($riissEst?->microred ?? 'Red Asistencial'),
                    'prestador'               => $bio->prestador ?? ($riissEst?->prestador ?? 'IPS'),
                    'tipologia_clasificacion' => $bio->tipoEstablecimiento?->nombre ?? ($riissEst?->tipologia_clasificacion ?? 'PUESTO SANITARIO'),
                    'latitude'                => $bio->latitud ?? $riissEst?->latitude,
                    'longitude'               => $bio->longitud ?? $riissEst?->longitude,
                    'area_gestion'            => $areaGestionNombre,
                    'situacion_inmueble'      => $bio->situacion_inmueble ?? $riissEst?->situacion_inmueble,
                    'sistema_hospitalario'    => $bio->sistema ?? $riissEst?->sistema_hospitalario,
                    'codigo'                  => $bio->codigo_sih ?? $riissEst?->codigo,
